...
export class RemotePlayer extends Player {
  constructor(scene, avatars, id, config) {
    ...
    this.id = id; this.config = config; this.targetPosition = new THREE.Vector3();
    this.targetRotation = 0; this.hasSnapshot = false; this.lastSeen = performance.now();
  }
  applySnapshot(snapshot) {
    ...
    if (!this.hasSnapshot) { this.root.position.copy(this.targetPosition); this.root.rotation.y = this.targetRotation; this.hasSnapshot = true; }
    if (snapshot.avatarId !== this.avatarRequestId) {
      this.avatarRequestId = snapshot.avatarId;
      this.setAvatar(snapshot.avatarId).catch((error) => console.warn('[Free Roam] Avatar remoto:', error));
    }
  }
  update(delta) {
